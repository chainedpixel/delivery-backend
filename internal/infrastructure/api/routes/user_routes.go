...
func RegisterUserRoutes(router *mux.Router, userHandler *handlers.UserHandler) {
	...
	router.HandleFunc("/users/{user_id}/roles", func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
	}).Methods(http.MethodOptions)
	...
	router.HandleFunc("/users/{user_id}/roles", userHandler.AssignRoleToUser).Methods(http.MethodPost)
}
